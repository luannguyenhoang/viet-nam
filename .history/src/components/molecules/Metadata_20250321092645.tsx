...
export default function Metadata({
  ...
  console.log("sos"+typeof data?.head, Array.isArray(data?.head));

  const metaTitle = Array.isArray(data?.head)
    ? data?.head?.find((tag: string) => tag.includes('property="og:title"'))?.match(/content="([^"]+)"/)?.[1]
    : title;
