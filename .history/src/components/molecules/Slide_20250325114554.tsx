    ...
    async function fetchCategoryId() {
      try {
        if (isAmThucPage) {
          ...
          const amThucRes = await fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/categories?slug=am-thuc`);
          const amThucData = await amThucRes.json();
          
          const regionRes = await fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/categories?slug=${slug}`);
  ...
  async function fetchPosts(catId: number | number[] | null) {
    try {
      ...
      if (Array.isArray(catId)) {
        // Nếu có nhiều category IDs
        apiUrl = `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/posts?_embed&categories=${catId.join(',')}`;
      } else {
        // Nếu chỉ có 1 hoặc không có category ID
        apiUrl = `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/posts?_embed${catId ? `&categories=${catId}` : ''}`;
      }
      ...
      // Lọc posts nếu đang ở trang ẩm thực và có nhiều categories
      const filteredPosts = Array.isArray(catId) && isAmThucPage
