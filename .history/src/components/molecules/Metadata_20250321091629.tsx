...
export default function Metadata({
  ...
  // Trích xuất dữ liệu từ API (nếu có) hoặc dùng giá trị mặc định
  const metaTitle = data?.head?.find((tag: string) => tag.includes('property="og:title"'))
    ?.match(/content="([^"]+)"/)?.[1] || title;
  const metaDescription = data?.head?.find((tag: string) => tag.includes('name="description"'))
    ?.match(/content="([^"]+)"/)?.[1] || description;
  const metaCanonical = data?.head?.find((tag: string) => tag.includes('property="og:url"'))
    ?.match(/content="([^"]+)"/)?.[1] || canonicalUrl;
  const metaImage = data?.head?.find((tag: string) => tag.includes('property="og:image"'))
    ?.match(/content="([^"]+)"/)?.[1] || ogImage;
  const metaRobots = data?.head?.find((tag: string) => tag.includes('name="robots"'))
