...
export default function SlideConten({ slug }: { slug: string }) {
  ...
  async function fetchPosts(catId: number | null, amThucId: number | null) {
    try {
      ...
      // Ensure both category IDs are used for filtering
      ...
      // Filter the results to ensure they belong to both categories
      const filteredPosts = result.filter((post: any) => {
        const postCategories = post.categories || [];
        return postCategories.includes(catId) && postCategories.includes(amThucId);
      });
