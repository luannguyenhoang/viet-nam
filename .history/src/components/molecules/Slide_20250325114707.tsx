...
export default function SlideConten({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        if (isAmThucPage) {
          // Nếu là trang ẩm thực, lấy cả 2 loại category
          const [amThucRes, regionRes] = await Promise.all([
            fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/categories?slug=am-thuc`),
            fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/categories?slug=${slug}`)
          ]);
          ...
          // Nếu không phải trang ẩm thực, chỉ lấy category miền
  ...
  async function fetchPosts(catIds: number[]) {
    try {
      ...
      // Lọc kết quả dựa vào điều kiện trang
      const filteredPosts = isAmThucPage && catIds.length > 1
