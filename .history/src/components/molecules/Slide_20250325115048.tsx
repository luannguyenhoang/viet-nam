...
export default function SlideConten({ slug }: { slug: string }) {
  ...
  // Tạo baseApiUrl để tái sử dụng
  const baseApiUrl = process.env.NEXT_PUBLIC_WORDPRESS_API_URL;

  // Sử dụng useCallback để tránh tạo lại hàm trong mỗi render
  const fetchCategoryBySlug = useCallback(async (categorySlug: string) => {
    const res = await fetch(`${baseApiUrl}/categories?slug=${categorySlug}`);
    if (!res.ok) throw new Error(`Không thể lấy thông tin category: ${categorySlug}`);
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      // Xác định các category IDs cần lấy
      if (isAmThucPage) {
        // Lấy cả category ẩm thực và miền
      ...
      // Fetch posts dựa theo categoryIds
      ...
      const filteredPosts = isAmThucPage
        ? data.filter(post => categoryIds.every(id => post.categories.includes(id)))
        : data.filter(post => 
