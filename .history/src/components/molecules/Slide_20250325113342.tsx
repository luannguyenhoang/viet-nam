...
export default function SlideConten({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    if (categoryId !== null) {
      // Only fetch posts if slug includes "am-thuc"
      if (slug.includes("am-thuc") && amThucId !== null) {
        fetchPosts(categoryId, amThucId);
      } else {
        fetchPosts(categoryId, null); // Fetch only by region category
      }
    }
  ...
  async function fetchPosts(catId: number | null, amThucId: number | null) {
    try {
      ...
      if (catId !== null) {
        ...
        if (amThucId !== null) {
          apiUrl += `&categories[]=${amThucId}`; // Only add if amThucId is not null
        }
      }
      ...
      const filteredPosts = result.filter((post: any) => {
        ...
        return amThucId ? postCategories.includes(catId) && postCategories.includes(amThucId) : postCategories.includes(catId);
      });
