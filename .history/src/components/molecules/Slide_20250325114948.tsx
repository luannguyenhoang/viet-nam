  ...
  async function fetchPosts(catIds: number[]) {
    try {
      ...
      if (isAmThucPage) {
        ...
        const res = await fetch(apiUrl);
        const data = await res.json();
        // Lọc để chỉ lấy bài thuộc đúng category miền và KHÔNG có category ẩm thực
        result = data.filter((post: any) => 
