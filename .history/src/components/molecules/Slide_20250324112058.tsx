...
export default function SlideConten({ slug }: { slug: string }) {
  ...
  // Đầu tiên, lấy ID của category từ slug
  useEffect(() => {
    ...
    if (slug) {
      ...
      // Nếu không có slug, tải tất cả bài viết
      fetchPosts(null);
    }
  }, [slug]);

  // Sau khi có ID category, lấy bài viết theo category đó
  useEffect(() => {
    if (categoryId !== null) {
  ...
  async function fetchPosts(catId: number | null) {
    try {
      ...
      // Nếu có categoryId, thêm vào query
      if (catId !== null) {
    ...
    <div>
      <Box mb={6} borderColor="gray.200" pb={4}>
        {data.length > 0 ? (
          ...
          <Box textAlign="center" py={10}>Không có bài viết nào.</Box>
        )}
      </Box>
    </div>
  );
