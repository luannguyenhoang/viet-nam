...
export default function CommentsPost({ slug }: { slug: any }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Phân loại bình luận thành cấu trúc cây
        ...
        allComments.forEach((comment: any) => {
          if (comment.parent === 0) {
            ...
            // Bình luận con - thêm vào replies của bình luận cha
  ...
  // Tạo component đệ quy để hiển thị bình luận và các phản hồi
  ...
  return (
    <VStack spacing={4} align="stretch" my={6}>
      ...
      {/* Danh sách bình luận với cấu trúc đa cấp */}
      {data && data.length > 0 ? (
        data.map((comment) => (
          <Box
            key={comment.id}
...
// Hàm đếm tổng số bình luận (bao gồm tất cả các cấp)
function countAllComments(comments: any[]): number {
  let count = 0;
  
  function countReplies(comment: any) {
    count++; // Đếm bình luận hiện tại
    if (comment.replies && comment.replies.length > 0) {
      comment.replies.forEach(countReplies); // Đệ quy đếm các phản hồi
    }
  }
