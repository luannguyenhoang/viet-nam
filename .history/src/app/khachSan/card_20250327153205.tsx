...
import { FaRulerCombined, FaUser, FaUtensils, FaWifi } from "react-icons/fa";

export default function CardZoom({ prop }: { prop: any }) {
  console.log(prop);

  // Kiểm tra xem dữ liệu có hợp lệ không
  const rooms = prop?.allKhachSan?.nodes?.[0]?.cardZoom?.roomCardChild || [];

  if (rooms.length === 0) {
    return (
      <Box textAlign="center" py={12}>
        <Text fontSize="lg" color="gray.600">
          Không có phòng khách sạn nào được tìm thấy.
        </Text>
      </Box>
    );
  }
              ...
              <CardBody p={0} position="relative" borderRadius={10}>
                ...
                >
                  ...
                  <HStack spacing={4} mt={4}>
                    <Link href={`/${room?.cardContent?.slug}`}>
