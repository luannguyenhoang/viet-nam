...
export async function getPageMetadata(slug: string): Promise<Metadata> {
  try {
    // Gọi API dựa vào slug của trang
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/${slug}`, { next: { revalidate: 3600 }});
    if (!res.ok) {
    ...
    // Tạo title và description mặc định dựa trên slug
    let defaultTitle = `${slug.charAt(0).toUpperCase() + slug.slice(1)} - Du lịch Việt Nam`;
    let defaultDescription = `Khám phá vẻ đẹp ${slug} Việt Nam`;
    
    // Trích xuất title và description từ dữ liệu
    ...
    return {
      title: `${slug.charAt(0).toUpperCase() + slug.slice(1)} - Du lịch Việt Nam`,
