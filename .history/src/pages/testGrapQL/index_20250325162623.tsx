...
// Định nghĩa các interface để type safety tốt hơn
interface ImageData {
  mediaItemUrl: string;
}
...
export default function GraphQLTest() {
  ...
  // Tách phần render hình ảnh ra component riêng
  const renderImages = () => {
    return data.allGraphQL.nodes.map((node, index) => (
      node?.graphqlItem?.graphqlChild?.a1.map((item, i) => {
