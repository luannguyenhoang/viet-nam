  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.log("CardZoom:", result.data?.allKhachSan?.nodes?.[0]?.cardZoom);
        console.log("RoomCardChild:", result.data?.allKhachSan?.nodes?.[0]?.cardZoom?.roomCardChild);
        
        result.data?.allKhachSan?.nodes?.[0]?.cardZoom?.roomCardChild?.forEach((room: any, index: number) => {
          console.log(`Room ${index + 1} photos:`, room?.cardContent?.photos);
          room?.cardContent?.photos?.forEach((photo, i) => {
            console.log(`Room ${index + 1}, photo ${i + 1}:`, photo);
            console.log(`Room ${index + 1}, photo ${i + 1} URL:`, photo?.child?.node?.mediaItemUrl);
          });
        });
                ...
                <CardBody p={0} position="relative" borderRadius={10}>
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <Box display="grid" gridTemplateColumns="repeat(3, 2fr)" gap={2}>
                        {room?.cardContent?.photos?.map((photo: any, photoIndex: number) => (
                          <Box key={photoIndex} width="full" height="16" overflow="hidden" borderRadius="md">
                            <Image
                              src={photo?.child?.node?.mediaItemUrl || "/quanCafe.jpg"}
