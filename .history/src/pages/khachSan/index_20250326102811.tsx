    ...
    async function fetchData() {
      try {
        ...
        result.data?.allKhachSan?.nodes?.[0]?.cardZoom?.roomCardChild?.forEach((room: any, index: number) => {
          console.log(`Room ${index + 1} photos:`, room?.cardContent?.photos?.child?.map(child => child?.node?.mediaItemUrl));
        });
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <HStack justify="space-between" spacing={2}>
                        {room?.cardContent?.photos?.child?.map((photo: any, photoIndex: number) => (
                          <Image
                            key={photoIndex}
                            src={photo?.node?.mediaItemUrl || "/quanCafe.jpg"}
