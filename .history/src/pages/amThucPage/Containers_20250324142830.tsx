...
export default function Containers({ session1 }: { session1: AmThuc }) {
      ...
      <Container maxW="container.xl">
        ...
        <Tabs variant="soft-rounded" colorScheme="red" isLazy>
          <TabList justifyContent="center" mb={8}>
            {foodCategories.map((session1.) => (
              <Tab
                key={session1..slug}
                _selected={{ color: "white", bg: session1..color }}
                mx={2}
                fontWeight="bold"
              >
                {session1..name}
